...
function downloadReadme({ query, limit, totalPossible, returned, fields, sourceRecordUrlRows }) {
  const lines = [
    ...
    "",
    "data file = data.csv",
    "citation file = CITATION.md",
    "source citation table = source_citations.csv",
...
async function createPhenobaseDownload({ query, limit }) {
  ...
  const dataSourceCounts = sourceCountsFromRecords(enrichedRecords);
  ...
  const buffer = createZip([
    ...
    { name: "CITATION.md", data: citationMarkdown(dataSourceCounts, accessDate) },
    { name: "source_citations.csv", data: sourceCitationsCsv(dataSourceCounts, accessDate, escapeCsvValue) },
    { name: "citation_and_data_use_policies.txt", data: citationAndPoliciesText(dataSourceCounts, accessDate) },
