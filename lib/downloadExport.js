...
function deriveInput(record) {
  const existing = String(record?.input || "").trim();
  ...
  const basis = String(record?.basisOfRecord || "").trim().toLowerCase();
